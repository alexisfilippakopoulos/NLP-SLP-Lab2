...
class SimpleSelfAttentionModel(nn.Module):
    ...
    def forward(self, x):
        B, T = x.shape
        tok_emb = self.token_embedding_table(x.clamp(0, self.token_embedding_table.num_embeddings - 1))  # (B,T,C)
        pos_emb = self.position_embedding_table(torch.arange(T, device=x.device).clamp(0, self.max_length - 1))  # (T,C)
...
class TransformerEncoderModel(nn.Module):
    def __init__(self, output_size, embeddings, max_length=60, n_head=3, n_layer=3):
        ...
        self.blocks = nn.Sequential(
            *[Block(n_head, dim // n_head, dim) for _ in range(n_layer)]
        )
    ...
    def forward(self, x):
        ...
        x = x.mean(dim=1)  # (B,C) - average pooling to get sentence embedding
        logits = self.output(x)  # (B,output_size)
